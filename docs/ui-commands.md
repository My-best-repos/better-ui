...
Five filesystem-only scanners that inspect frontend UI structure, colors, component conventions, typography, and spacing. Unlike the main ESLint scanner, these work on any project without configuration.
...
From the TUI (after `/menu`): type any of `/ui-audit`, `/ui-colors`, `/ui-standards`, `/ui-typography`, or `/ui-spacing` at the prompt.

## Considerations

- All five scanners skip `node_modules`, `dist`, `.git`, `build`, `.next`, `.cache`, and `coverage` directories.
...
- All five are pure filesystem scanners — no ESLint, no configuration needed.
